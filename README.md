# Welcome To Your New XHRISTIAN Community

This repository serves as the gateway to the **XHRISTIAN Community Wiki**.

## 📖 Access the Wiki

The primary community documentation, including the project's narrative, commandments, and guidelines, is hosted in a dedicated Wiki for a better reading experience.

**➡️ [Visit the XHRISTIAN Community Wiki](https://github.com/xhristiancoin/xhristian-community/wiki)**

## 🛠️ About This Repository

This `xhristian-community` repository exists to enable the Wiki feature. The Wiki is the main source of living documents, while the core, immutable protocol specifications are maintained in the [`xhristian-protocol`](https://github.com/xhristiancoin/xhristian-protocol) repository.

The Wiki is designed to be intuitive for all participants. You can navigate its contents using the sidebar.

---
*For formal specifications and on-chain verification, please refer to the `xhristian-protocol` repository.*